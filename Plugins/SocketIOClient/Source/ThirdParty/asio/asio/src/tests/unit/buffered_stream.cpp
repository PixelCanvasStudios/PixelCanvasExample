...
typedef asio_sockio::buffered_stream<
    asio_sockio::ip::tcp::socket> stream_type;

void write_some_handler(const asio_sockio::error_code&, std::size_t)
{
}

void flush_handler(const asio_sockio::error_code&, std::size_t)
{
}

void fill_handler(const asio_sockio::error_code&, std::size_t)
{
}
...
void test_sync_operations()
{
  ...
  asio_sockio::ip::tcp::acceptor acceptor(io_context,
      asio_sockio::ip::tcp::endpoint(asio_sockio::ip::tcp::v4(), 0));
  asio_sockio::ip::tcp::endpoint server_endpoint = acceptor.local_endpoint();
  server_endpoint.address(asio_sockio::ip::address_v4::loopback());
  ...
  const asio_sockio::const_buffer write_buf = asio_sockio::buffer(write_data);

  std::size_t bytes_written = 0;
  while (bytes_written < sizeof(write_data))
  {
    bytes_written += client_socket.write_some(
        asio_sockio::buffer(write_buf + bytes_written));
    client_socket.flush();
  }

  char read_data[sizeof(write_data)];
  const asio_sockio::mutable_buffer read_buf = asio_sockio::buffer(read_data);

  std::size_t bytes_read = 0;
  while (bytes_read < sizeof(read_data))
  {
    bytes_read += server_socket.read_some(
        asio_sockio::buffer(read_buf + bytes_read));
  }
  ...
  bytes_read = client_socket.read_some(
      asio_sockio::buffer(read_buf), error);

  ASIO_CHECK(bytes_read == 0);
  ASIO_CHECK(error == asio_sockio::error::eof);
    ...
    std::size_t* total_bytes_written)
{
  ASIO_CHECK(!e);
  if (e)
    throw asio_sockio::system_error(e); // Terminate test.
  *total_bytes_written += bytes_transferred;
}

void handle_flush(const asio_sockio::error_code& e)
{
  ASIO_CHECK(!e);
}
...
void handle_read_eof(const asio_sockio::error_code& e,
    std::size_t bytes_transferred)
{
  ASIO_CHECK(e == asio_sockio::error::eof);
...
void test_async_operations()
{
  ...
  server_socket.close();
  client_socket.async_read_some(asio_sockio::buffer(read_buf), handle_read_eof);
}
