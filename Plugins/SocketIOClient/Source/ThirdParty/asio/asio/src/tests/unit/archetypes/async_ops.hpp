...
{
  typedef typename asio_sockio::async_completion<CompletionToken,
    void()>::completion_handler_type handler_type;

  asio_sockio::async_completion<CompletionToken,
    void()> completion(token);

  typename asio_sockio::associated_allocator<handler_type>::type a
    = asio_sockio::get_associated_allocator(completion.completion_handler);

  typename asio_sockio::associated_executor<handler_type>::type ex
    = asio_sockio::get_associated_executor(completion.completion_handler);
...
template <typename CompletionToken>
ASIO_INITFN_RESULT_TYPE(CompletionToken, void(asio_sockio::error_code))
async_op_ec_0(bool ok, ASIO_MOVE_ARG(CompletionToken) token)
{
  typedef typename asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code)>::completion_handler_type handler_type;

  asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code)> completion(token);
  ...
  else
  {
    ex.post(
        bindns::bind(
          ASIO_MOVE_CAST(handler_type)(completion.completion_handler),
          asio_sockio::error_code(asio_sockio::error::operation_aborted)), a);
  }
...
{
  typedef typename asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int)>::completion_handler_type
      handler_type;

  asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int)> completion(token);
...
template <typename CompletionToken>
ASIO_INITFN_RESULT_TYPE(CompletionToken,
    void(asio_sockio::error_code, int, double))
async_op_ec_2(bool ok, ASIO_MOVE_ARG(CompletionToken) token)
{
  typedef typename asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int, double)>::completion_handler_type
      handler_type;

  asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int, double)> completion(token);
...
template <typename CompletionToken>
ASIO_INITFN_RESULT_TYPE(CompletionToken,
    void(asio_sockio::error_code, int, double, char))
async_op_ec_3(bool ok, ASIO_MOVE_ARG(CompletionToken) token)
{
  typedef typename asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int, double, char)>::completion_handler_type
      handler_type;

  asio_sockio::async_completion<CompletionToken,
    void(asio_sockio::error_code, int, double, char)> completion(token);
  ...
  if (ok)
  {
    ex.post(
        bindns::bind(
          ASIO_MOVE_CAST(handler_type)(completion.completion_handler),
          asio_sockio::error_code(), 42, 2.0, 'a'), a);
  }
  else
  {
    ex.post(
