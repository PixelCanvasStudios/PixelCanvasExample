...
public:
  ...
  ASIO_DECL asio_sockio::error_code set_verify_mode(
      verify_mode v, asio_sockio::error_code& ec);

  // Set the peer verification depth.
  ASIO_DECL asio_sockio::error_code set_verify_depth(
      int depth, asio_sockio::error_code& ec);

  // Set a peer certificate verification callback.
  ASIO_DECL asio_sockio::error_code set_verify_callback(
  ...
  ASIO_DECL want shutdown(asio_sockio::error_code& ec);

  // Write bytes to the SSL session.
  ASIO_DECL want write(const asio_sockio::const_buffer& data,
      asio_sockio::error_code& ec, std::size_t& bytes_transferred);

  // Read bytes from the SSL session.
  ASIO_DECL want read(const asio_sockio::mutable_buffer& data,
      asio_sockio::error_code& ec, std::size_t& bytes_transferred);

  // Get output data to be written to the transport.
  ASIO_DECL asio_sockio::mutable_buffer get_output(
      const asio_sockio::mutable_buffer& data);

  // Put input data that was read from the transport.
  ASIO_DECL asio_sockio::const_buffer put_input(
      const asio_sockio::const_buffer& data);

  // Map an error::eof code returned by the underlying transport according to
  // the type and state of the SSL session. Returns a const reference to the
  // error code object, suitable for passing to a completion handler.
  ASIO_DECL const asio_sockio::error_code& map_error_code(
...
private:
  ...
  ASIO_DECL static asio_sockio::detail::static_mutex& accept_mutex();
