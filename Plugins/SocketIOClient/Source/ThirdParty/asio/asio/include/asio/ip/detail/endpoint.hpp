...
public:
  ...
  ASIO_DECL endpoint(const asio_sockio::ip::address& addr,
      unsigned short port_num) ASIO_NOEXCEPT;
  ...
  asio_sockio::detail::socket_addr_type* data() ASIO_NOEXCEPT
  {
    return &data_.base;
  }

  // Get the underlying endpoint in the native type.
  const asio_sockio::detail::socket_addr_type* data() const ASIO_NOEXCEPT
  {
    return &data_.base;
  }
  ...
  {
    if (is_v4())
      return sizeof(asio_sockio::detail::sockaddr_in4_type);
    else
      return sizeof(asio_sockio::detail::sockaddr_in6_type);
  }
  ...
  ASIO_DECL asio_sockio::ip::address address() const ASIO_NOEXCEPT;

  // Set the IP address associated with the endpoint.
  ASIO_DECL void address(
      const asio_sockio::ip::address& addr) ASIO_NOEXCEPT;
...
private:
  ...
  union data_union
  {
    asio_sockio::detail::socket_addr_type base;
    asio_sockio::detail::sockaddr_in4_type v4;
    asio_sockio::detail::sockaddr_in6_type v6;
  } data_;
};
