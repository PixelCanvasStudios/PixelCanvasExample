...
kqueue_reactor::kqueue_reactor(asio_sockio::execution_context& ctx)
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  {
    asio_sockio::error_code error(errno,
        asio_sockio::error::get_system_category());
    asio_sockio::detail::throw_error(error);
  }
}
  ...
  if (fork_ev == asio_sockio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      asio_sockio::error_code ec(errno,
          asio_sockio::error::get_system_category());
      asio_sockio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          asio_sockio::error_code ec(errno,
              asio_sockio::error::get_system_category());
          asio_sockio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    asio_sockio::error_code ec(errno,
        asio_sockio::error::get_system_category());
    asio_sockio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
