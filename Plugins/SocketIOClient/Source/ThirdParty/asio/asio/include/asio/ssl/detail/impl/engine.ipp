  ...
  : ssl_(::SSL_new(context))
{
  if (!ssl_)
  {
    ...
    asio_sockio::detail::throw_error(ec, "engine");
  }
...
asio_sockio::error_code engine::set_verify_mode(
...
asio_sockio::error_code engine::set_verify_depth(
...
asio_sockio::error_code engine::set_verify_callback(
    verify_callback_base* callback, asio_sockio::error_code& ec)
{
  if (SSL_get_app_data(ssl_))
    delete static_cast<verify_callback_base*>(SSL_get_app_data(ssl_));
...
engine::want engine::handshake(
    stream_base::handshake_type type, asio_sockio::error_code& ec)
{
  return perform((type == asio_sockio::ssl::stream_base::client)
      ? &engine::do_connect : &engine::do_accept, 0, 0, ec, 0);
}

engine::want engine::shutdown(asio_sockio::error_code& ec)
{
  return perform(&engine::do_shutdown, 0, 0, ec, 0);
}

engine::want engine::write(const asio_sockio::const_buffer& data,
    asio_sockio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
    ec = asio_sockio::error_code();
...
engine::want engine::read(const asio_sockio::mutable_buffer& data,
    asio_sockio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
...
asio_sockio::mutable_buffer engine::get_output(
    const asio_sockio::mutable_buffer& data)
{
  int length = ::BIO_read(ext_bio_,
      data.data(), static_cast<int>(data.size()));

  return asio_sockio::buffer(data,
      length > 0 ? static_cast<std::size_t>(length) : 0);
}

asio_sockio::const_buffer engine::put_input(
...
const asio_sockio::error_code& engine::map_error_code(
    asio_sockio::error_code& ec) const
{
  // We only want to map the error::eof code.
  if (ec != asio_sockio::error::eof)
    return ec;

  // If there's data yet to be read, it's an error.
  if (BIO_wpending(ext_bio_))
  {
    ec = asio_sockio::ssl::error::stream_truncated;
    return ec;
  }
...
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
asio_sockio::detail::static_mutex& engine::accept_mutex()
{
  static asio_sockio::detail::static_mutex mutex = ASIO_STATIC_MUTEX_INIT;
  return mutex;
}
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)

engine::want engine::perform(int (engine::* op)(void*, std::size_t),
    void* data, std::size_t length, asio_sockio::error_code& ec,
    std::size_t* bytes_transferred)
{
  ...
  {
    ec = asio_sockio::error_code(sys_error,
        asio_sockio::error::get_ssl_category());
    return want_nothing;
  }
...
{
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
  asio_sockio::detail::static_mutex::scoped_lock lock(accept_mutex());
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
