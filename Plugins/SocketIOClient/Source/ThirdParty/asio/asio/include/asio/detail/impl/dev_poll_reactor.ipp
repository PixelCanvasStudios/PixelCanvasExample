...
dev_poll_reactor::dev_poll_reactor(asio_sockio::execution_context& ctx)
  : asio_sockio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
{
  asio_sockio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, asio_sockio::error::operation_aborted);
}
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    asio_sockio::error_code ec(errno,
        asio_sockio::error::get_system_category());
    asio_sockio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
