...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = asio_sockio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = asio_sockio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = asio_sockio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return asio_sockio::buffer(buffer_ + total_consumed_, max_size);
  }
...
public:
  ...
  boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    ...
    result[0] = asio_sockio::buffer(result[0] + total_consumed_, max_size);
    result[1] = asio_sockio::buffer(
...
class consuming_buffers<Buffer, null_buffers, const mutable_buffer*>
  : public asio_sockio::null_buffers
{
public:
